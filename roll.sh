...
${yell}Usage: ./astley.sh [OPTIONS...]
${purp}OPTIONS : $yell
  help   - Show this message.
  inject - Append to $purp$USER$yell's bashrc. (Recommended :D)\
"
}
...
if has? afplay; then
  ...
  [ -f /tmp/roll.s16 ] || gimme $audio_raw >/tmp/roll.s16
  afplay /tmp/roll.s16 &
elif has? aplay; then
  # On Linux, if |aplay| available, stream raw sound.
  gimme $audio_raw | aplay -Dplug:default -q -f S16_LE -r 8000 &
elif has? play; then
...
# Sync FPS to reality as best as possible. Mac's freebsd version of date dosen't
...
try:
  ...
  pass
EOF
) < <(gimme $video | bunzip2 -q 2> /dev/null)
